...
var miwi_poi_xml3d_requests = {}; // ["UUID": {lon: <lon>, lat: <lat>},...]
...
(function ( namespace ) {
    var log = wex.Util.log, map, geocoder, homeMarker, positionMarker, 
		...
		BACKEND_ADDRESS_POI = "ws://130.231.12.82:9000",
		BACKEND_ADDRESS_3D = "ws://130.231.12.82:9001",
...
*/
/* Live ... */
    function connectBackend_POI() {
        try {
            if ( window.WebSocket !== undefined ) {
                webSocket_POI = new window.WebSocket( BACKEND_ADDRESS_POI );
            }
            else {
        ...
        webSocket_POI.onopen = function () {
            console.log( "WebSocket_POI opened" );
            searchPOIs();
        }.bind( this );

        webSocket_POI.onmessage = function ( msg ) {
            console.log("Got POI msg: " + msg.data);
            parsePOIMsg( msg.data );
        }.bind( this );

        webSocket_POI.onclose = function () {
            setTimeout( function () {
                connectBackend_POI();
            }, 5000 );
        };
...
/* Blank ... 
    function connectBackend_POI() {
        webSocket_POI = {};
        webSocket_POI.onopen = function () {
            console.log( "WebSocket POI dummy opened" );
            searchPOIs();
        }.bind( this );
		...
		webSocket_POI.send = function (query) {
		    console.log("Sending: " + query);
			// create immediate response with constant data
			webSocket_POI.onmessage({data: JSON.stringify(
                 {
                     "pois": {
		...
		webSocket_POI.onopen();
    }
 
 ... end Blank */

/* Live... */
    function connectBackend_3D() {
        try {
            if ( window.WebSocket !== undefined ) {
                webSocket_3D = new window.WebSocket( BACKEND_ADDRESS_3D );
            }
            else {
                alert( "This Browser does not support WebSockets." );
                return;
        ...
        webSocket_3D.onopen = function () {
            console.log( "WebSocket_3D opened" );
        }.bind( this );

        webSocket_3D.onmessage = function ( msg ) {
            console.log("Got 3D msg: " + msg.data);
            parse3DMsg( msg.data );
        }.bind( this );

        webSocket_3D.onclose = function () {
            setTimeout( function () {
                connectBackend_3D();
            }, 5000 );
        };
...
/* Blank ... 
    function connectBackend_3D() {
        webSocket_3D = {};
        webSocket_3D.onopen = function () {
            console.log( "webSocket_3D dummy opened" );
        }.bind( this );
		...
		webSocket_3D.send = function (query) {
		    console.log("Sending 3D: " + query);
			// create immediate response with constant data
			webSocket_3D.onmessage({data: JSON.stringify(
                {
                    "xml3d_models": {
                        "8e57d2e6-f98f-4404-b075-112049e72346": [ // entities
                            { // entity 1
 ...
 ... end Blank */
    ...
    function searchPOIs( lat, lng ) {
        ...
        webSocket_POI.send( JSON.stringify( {lat: lat, lon: lng, 
...
//console.log(poiWindow.content);
        ...
        miwi_poi_xml3d_requests[uuid] = {lon: data.location.lon, 
                lat: data.location.lat};
        webSocket_3D.send( JSON.stringify( {points: [uuid], id: queryID} ) );
        queryID++;
